...
export class AppsAdd extends React.Component {
  ...
  addData = async () => {
    ...
    try {
      await fetch(`http://prod.7tvpyhipq3.us-east-2.elasticbeanstalk.com/createApp`, {
        method: 'POST',
        headers: {
    ...
    console.log('image upload selcted file name', this.state.appname);
    try {
      console.log('image upload selcted file name', this.state.appname);
      const data = new FormData();
      data.append('file', this.state.selectedFile);
      fetch(`http://prod.7tvpyhipq3.us-east-2.elasticbeanstalk.com/upload/${this.state.appname}`, {
        method: 'POST',
        body: data,
      })
    .then(res => res.json())
